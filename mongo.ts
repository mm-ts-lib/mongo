...
export class Mongo<T extends IDbSchemas>{
    ...
    constructor(dbCollectionsDefine: T) {
    ...
    public async connect() {
        ...
        this._client = await MongoClient.connect(dbConfig.url, dbConfig.options);
        this._db = await this._client.db(mod.name);
    ...
    private async _ensureSchemaCollections() {
        ...
        // 创建新的已定义colls
        for (const newColl of _.difference(Object.keys(this._dbCollectionsDefine), Object.keys(curColls))) {
